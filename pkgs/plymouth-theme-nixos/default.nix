...
let
  plymouthConfig = writeTextFile {
    name = "nixos.plymouth";
    text = ''
      [Plymouth Theme]
      Name=NixOS
      Description=NixOS splash screen.
      ModuleName=script
  ...
  plymouthScript = writeTextFile {
    name = "nixos.script";
    text = ''
      # NixOS Plymouth Theme Script
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    # Use the generated config and script files with correct paths
    cp ${plymouthConfig} $themeDir/nixos.plymouth
    cp ${plymouthScript} $themeDir/nixos.script

    # Replace $out in the plymouth file with actual path
  ...
  meta = with lib; {
    description = "Custom NixOS Plymouth theme with progress bar and password dialog";
